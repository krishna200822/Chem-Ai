...
(() => {
  ...
  function saveResult() {
    ...
    items.unshift({ id: crypto.randomUUID?.() || String(Date.now()), createdAt: new Date().toISOString(), reactants: input, conditions, result: state.lastResult });
  ...
  function clearAnalysis() {
    const i = findInput(); const c = findConditions();
    if (i) i.value = "";
    if (c) c.value = "";
  ...
  function wire() {
    ...
    if (predictButton && !predictButton.dataset.chemaiWired) {
      ...
      predictButton.addEventListener("click", e => { e.preventDefault(); predict(); });
    }

    document.addEventListener("click", e => {
      const target = e.target.closest?.("[data-action],[data-copy]");
      if (!target) return;
      if (target.dataset.action === "save-result") saveResult();
      if (target.dataset.action === "copy-analysis") copyAnalysis();
      if (target.dataset.action === "new-analysis") clearAnalysis();
      if (target.dataset.copy === "equation") {
        const eq = state.lastResult?.reaction?.equation || state.lastResult?.prediction?.reaction?.equation || "";
        navigator.clipboard?.writeText(eq).then(() => notify("Equation copied."));
      }
    }, { once: false });
    ...
    $$("button").forEach(b => {
      ...
      if (/example|ethanol|sodium|hcl|naoh|ethene|brom/i.test(t) && !b.dataset.chemaiExample) {
        ...
        b.addEventListener("click", () => {
          const i = findInput();
          if (!i) return;
          if (/ethanol.*sodium/i.test(t)) i.value = "Ethanol + sodium";
          else if (/hcl.*naoh/i.test(t)) i.value = "HCl + NaOH";
          else if (/ethene.*br/i.test(t)) i.value = "Ethene + Br2";
          else if (/acetic.*ethanol/i.test(t)) i.value = "Acetic acid + ethanol";
